...
function _psyl2kron(A::Vector{MT1}, B::Vector{MT2},
                    ) where {MT1<:AbstractMatrix{T}, MT2<:AbstractMatrix{T}} where {T}
    K = length(A)
    p1 = checksquare(A[1])
    p2 = checksquare(B[1])
    pp = p1*p2
...
function _pgsyl2kron(A::Vector{MT1},B::Vector{MT2},C::Vector{MT3},D::Vector{MT4},
                     ) where {MT1<:AbstractMatrix{T}, MT2<:AbstractMatrix{T},
                              MT3<:AbstractMatrix{T}, MT4<:AbstractMatrix{T},
                              } where {T}
    K = length(A)
    p1 = checksquare(A[1])
    p2 = checksquare(B[1])
